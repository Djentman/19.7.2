...
def test_post_add_new_pet_without_photo(name='Лысый', animal_type='тоже кот', age=12):
    _, auth_key = pf.get_api_key(valid_email, valid_password)
...
def test_get_all_pets_with_invalid_filter(filter='not_my_pets'):
    ...
    assert status == 400 or 401 or 402 or 403 or 404 or 405
# 3
def test_post_add_new_pet_with_invalid_name(name='fghfhdhdshfjhajavajfjhajhfajhfhafbjhabfjkbajhbafhjasjahjbjhafsjhbfsjh', animal_type='кот', age='1', pet_photo='images/IMG_8521.jpeg'):
    pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.post_add_new_pet(auth_key, name, animal_type, age, pet_photo)
    assert status == 400 or 401 or 402 or 403 or 404 or 405
# 4
def test_post_add_new_pet_with_none_name(name='', animal_type='кот', age='1', pet_photo='images/IMG_8522.jpeg'):
    pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.post_add_new_pet(auth_key, name, animal_type, age, pet_photo)
    assert status == 400 or 401 or 402 or 403 or 404 or 405
